  ...
  <div class="table-responsive text-nowrap">
    <div class="container">
      ...
      <div class="justify-content-center d-flex my-2">
        <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#addProker">
          <span class="tf-icons bx bx-plus bx-18px me-2"></span>
          Add
        </button>
        @include('content.kegiatan.add/add-proker')
      ...
      <table id="example" class="table table-striped">
        ...
        <tbody class="table-border-bottom-0">
          ...
          <tr>
            ...
            <td>{{ $row -> devisi }}</td>
            <td>{{ ucwords(strtolower($row->kegiatan)) }}</td>
            <td>{{ $row -> tgl }}</td>
            <td>{{ $row -> status }}</td>
            <td class="whitespace-nowrap py-4 px-3 text-center text-sm font-medium">
              ...
              <form action="{{ route('destroy_proker', $row->id) }}" method="POST" style="display: inline;">
                @csrf
                @method('DELETE')
              ...
              @include('content.kegiatan.edit/edit-proker')
            </td>
          </tr>
          @endforeach
        </tbody>
